...
module FinalCutServer
  ...
  class Client
    ...
    def run(cmd, options = {}, args = [])
      puts cmd if FinalCutServer.debug
    ...
    def create_asset(source_file_path, source_file_name, device_addr, description = nil, keywords = nil, project_addr = nil, asset_type = "pa_asset_media", remove_original_file = false, trigger_analyze = true)
      ...
      puts file_full_path if FinalCutServer.debug
    ...
    #
    # Uses net/ssh and net/scp to copy a file over an scp channel to the remote server where final cut server is located.
    #
    #
    def ssh_scp(local_file, remote_file)
